...
//  float accelG[3];  // Stores the real accel value in g's
  ...
  int accelCount[3];  // Stores the 12-bit signed value
  readAccelData(accelCount);  // Read the x/y/z adc values
  x_data = (float) accelCount[0] / ((1<<12)/(2*GSCALE));
  return(x_data); 
    }
  ...
  int accelCount[3];  // Stores the 12-bit signed value
  readAccelData(accelCount);  // Read the x/y/z adc values
  y_data = (float) accelCount[1] / ((1<<12)/(2*GSCALE));
  return(y_data); 
    }
  ...
  int accelCount[3];  // Stores the 12-bit signed value
  readAccelData(accelCount);  // Read the x/y/z adc values
  z_data = (float) accelCount[2] / ((1<<12)/(2*GSCALE));
  return(z_data); 
    }
  ...
  int accelCount[3];  // Stores the 12-bit signed value
  readAccelData(accelCount);  // Read the x/y/z adc values
  x_data = (float) accelCount[0] / ((1<<12)/(2*GSCALE))*90;
  if(x_data>90)
  { x_data=90;}
   if(x_data<-90)
  { x_data=-90;}
  ...
  int accelCount[3];  // Stores the 12-bit signed value
  readAccelData(accelCount);  // Read the x/y/z adc values
  y_data = (float) accelCount[1] / ((1<<12)/(2*GSCALE))*90;
  if(y_data>90)
  { y_data=90;}
   if(y_data<-90)
  { y_data=-90;}
